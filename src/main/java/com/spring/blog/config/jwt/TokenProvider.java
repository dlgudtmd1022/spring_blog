...
@RequiredArgsConstructor // 맴버변수를 활용한 생성자 자동 생성
public class TokenProvider {
    ...
    private String makeToken(Date expiry, User user){
        Date now = new Date(); // 현재 시간으로 트콘 발급 날짜 저장할 준비

        return Jwts.builder() // 빌더패턴으로 포튼 생성
                .setHeaderParam(Header.TYPE, Header.JWT_TYPE) // 헤어타입은 JWT
    ...
    // 검증 로직은 유효한지, 혹은 유효하지 않은지만 리턴하면 되므로 boolean을 리텁합니다.
    // 발급된 토근을 입력받았을때, 유효하면 true, 아니면 false를 리텁
    public boolean validToken(String token){
        try{
            Jwts.parser() // 토큰을 먼저 파싱(읽어들임)하고
                    .setSigningKey(jwtProperties.getSecretKey()) // 복호화시에는 함호화때 사용한 비밀키를 입력
                    .parseClaimsJws(token); // 검증 타켓이 되는 토큰 입력
    ...
    public Authentication getAuthentication(String token){ // 토큰 입력시 인증정보 읽어오기
        Claims claims = getClaims(token); // getClaims()는 아래에 추가호 작성할것임
