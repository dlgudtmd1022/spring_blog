...
public class TokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    // 토근제공자 내부의 validToken메서드를 이용해서 토큰의 유효성을 검증할 예정이므로 의존성 주입
    private final TokenProvider tokenProvider;

    // 토큰은 RequestHeader에 포함되서 들어오므로, 해당 요소를 얻어낼 수 있도록 명칭 상수화
    private final static String HEADER_AUTHORIZATION = "Authorization";
    // 토큰기반 인증시 실제 토큰값 앞에 접주어로 붙는 BEARER 제거를 위한 상수
    private final static String TOKEN_PREFFIX = "Bearer ";

    // 아래 doFilterInternal은 매 요청마다 실행
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        // 토근은 요청헤더를 이용해서 제출되기 떄문에, request에서 header를 얻어와야 합니다.
    ...
    private String getAccessToken(String authorizationHeader) {
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith(TOKEN_PREFFIX)) {
            return authorizationHeader.substring(TOKEN_PREFFIX.length()); // 앞 7글자(Bearer )를 소가한 값만 리턴
        }
        return null;
    }
}
