...
public class BasicSecurityConfig { // 베이직 방식 인증을 사용하도록 설정하는 파일
    ...
    // 장작 파일이나 .jsp 파일 등 스프링 시큐리티가 정굑되지 않을 영역 설정하기.
    @Bean // @Configyration 어노테이션 붙은 클래스 내부 메서드가 리턴하는 자료는 자동으로 빈에 등록됩니다.
    public WebSecurityCustomizer configure(){
        return web -> web.ignoring() // 시큐리티 적용을 안 할 경로
                .requestMatchers("/static/**")    // 기본 경로는 src/main/java/resources로 잡히고
                                                    // 추루 설정할 정적자원 저장 경로에 보안을 풀었음.
                .dispatcherTypeMatchers(DispatcherType.FORWARD);
                                                    // MVC방식에서 뷰단 파일을 로딩하는 것을 보안범위에서 해제.
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        return http.
                authorizeRequests(authorzationConfig -> {
                    authorzationConfig
                ...
                .csrf(AbstractHttpConfigurer::disable) // (csrfConfig -> { csrfConfig.disable();})
                .build();

//                authorizeRequests() // 인증, 인가 설정 시작부에 사용한느 메서드
//                .requestMatchers("/login", "/signup", "/user")
//                .permitAll() // 위 경로들은 인증없이 접속 가능
//                .anyRequest() // 위에 적힌 경로 말고는
//                .authenticated() // 로그인 필수임.
//                .and() // 다음 설정으로 넘어가기
//                .formLogin() // 로그인 폼으로 로그인제어
