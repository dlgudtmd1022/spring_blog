...
<body>
    <div class="container">
        ...
        <div class="row">
            <!-- 비동기 form의 경우는 목적지로 이동하지 않고 페이지 내에서 처리가 되므로 action을 가지지 않습니다.
                 그리고 제출버튼도 제출기능을 막고 fetch 요청만 넣습니다.-->
                <div class="col-3">
    ...
    <script>
                    ...
                    replies.map((reply, i) => { // 첫 파라미터 : 반복대상자료, 두번쨰 파라미터 : 순번
                        str += `<h3>\${i + 1}번째 댓글 || 글쓴이 : \${reply.replyWriter},
                                댓글내용 : \${reply.replyContent}
                                    <span class="deleteReplyBtn" data-ro="${reply.replyId}">
                                        [삭제]
                                    </span>
                                </h3>`;
                         });
        ...
        // 해당 함수 실행시 비공디 폼에 작성된 글쓴이, 내용으로 댓글 입력
        function insertReply() {
            ...
            fetch(url,{
                ...
                headers : { // header에는 보내는 데이터의 자료형에 대해서 기술
                    // json 데이터를 요청과 함꼐 전달, @RequestBody를 입력받는 로직에 추가
